...
const query = req.query;
 ...
 if(result?.success){
    res.redirect(`${envVar.SSL.SSL_SUCCESS_FRONTEND_URL}?transactionId=${query.transactionId}&message=${result.message}&amount=${query.amount}&status=${query.status}`)
 }
});
...
const query = req.query;
 ...
 if(!result?.success){
    res.redirect(`${envVar.SSL.SSL_FAIL_FRONTEND_URL}?transactionId=${query.transactionId}&message=${result?.message}&amount=${query.amount}&status=${query.status}`)
 } 
});
